...
public class playername extends Activity {
    ...
    protected void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        NhapTen = findViewById(R.id.UserName);
        BatDau = findViewById(R.id.Start);
        TroVe = findViewById(R.id.Back);

        final MediaPlayer mediaPlayer = MediaPlayer.create(this, R.raw.buttonclick);

        BatDau.setOnClickListener(v -> {
            mediaPlayer.start();
            String name = NhapTen.getText().toString().trim();

            if (name.isEmpty()) {
                NhapTen.setError("Vui lòng nhập tên");
                return;
            }
